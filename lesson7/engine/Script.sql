DROP DATABASE IF exists lesson_seventh;
CREATE DATABASE lesson_seventh;
USE lesson_seventh;
...
-- INSERT INTO users (`firstname`, `lastname`, `email`, `pass`, `created_at`)
-- VALUES 
...
select * from users where users.email like ' Petr@mail.ru'


select distinct product_size.sizes,
product_quantity.product_id
FROM product_size 
JOIN product_quantity 
ON product_quantity.size_id = product_size.id 
WHERE product_quantity.quantity>0 
AND product_quantity.product_id = 1;

SELECT product_quantity.quantity, 
product_quantity.product_id 
FROM product_quantity
join product_colors 
on product_colors.colors = 'blue' 
and product_colors.id = product_quantity.color_id 
join product_size on product_size.sizes = 'L'
and product_size.id = product_quantity.size_id
WHERE  product_quantity.product_id = 5;
